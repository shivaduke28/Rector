...
- 1行を複数行に増やすのも同じ手（`insertBefore` で新しい行を入れてから元の行を `delete`）。`replace` に改行を渡すとこの手順を促すエラーが返る
- 行IDと本文の対応は `readPage ... | jq -r '.lines[] | .id + "\t" + .text'` で並べると組みやすい。opsの組み立て自体をスクリプトにするとタブや改行の取り回しが安全
- **本文からlineIDを引く辞書を作るときは重複行に注意。** 同じ文字列の行が複数あると後勝ちで潰れる（`Camera Node` は Input と Output の両方に `+ カメラごとに異なる` がある）。位置で選ぶか、前後の行と併せて特定する
- ときどき 503 が返る。opsは冪等ではないので、**submitが通ったかを確認してから**リトライする
...
2. **調査を並列化する。** ページをテーマ別（ノード系 / 入力・操作系 / シーン・設定系 / メタ・bug系）に分け、サブエージェントに「担当ページを `readPage` で読む + 対応コードを読む → keep / 要更新 / 廃止 の判定と修正案」を出させる。**調査エージェントには書き込みを禁止する**（読み取り専用と明示する）
   - **レポートが揃うまで書き込みを始めない。** 先に書くと、あとから届いた指摘で公開済みのページを直すことになる
   - **レポートも裏を取る。** サブエージェントの指摘にも誤りが混ざる（実在するフォントのパスを「存在しない」と報告された例がある）。修正を投げる前に自分でファイルを見る
